...
## merge subject_test and subject_train and Y_test en train & add labels
subject_file <- rbind(subject_test, subject_train)
names(subject_file) <- "subject"
activities <- rbind(Y_test, Y_train)
names(activities) <- "activity"

## merge subject_file, activities, data (X_testtrain) on colums to total
...
## second part step 5, create new file first rename to seconddataset: 
...
write.table(tidy_dataset_2, file = "~/cursussen/R/getting and cleaning data/tidydataset.txt", row.name=FALSE)
